...
Keyboard, mouse movement/click, and touchscreen input quit mpv. Physical activity and a monotonic 30-second timer drive the normal idle cycle, avoiding false wake or re-arm events when mpv or Chromium resets X11's idle counter. X idle is used only as a fallback when Linux input devices are unavailable and never dismisses a running video. As soon as the screensaver begins, Chromium replaces its current tab with the configured content URL through its loopback-only DevTools endpoint. The existing browser window stays behind the video; on wake, only the video closes, revealing the already-fresh root page immediately. An unexpected mpv failure is retried with a delay. A missing or corrupt file is reported in status and logs without disturbing web playback or causing a rapid restart loop.
