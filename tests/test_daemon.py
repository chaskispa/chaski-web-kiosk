...
from chaski_player.daemon import PlayerDaemon, PlayerState
...
class DaemonTests(unittest.TestCase):
    def test_start_is_idempotent_while_chromium_runs(self) -> None:
        ...
        ), mock.patch("chaski_player.daemon.subprocess.Popen", return_value=process) as popen:
            daemon.start_chromium()
            daemon.start_chromium()
        popen.assert_called_once()
        self.assertEqual(daemon.state, PlayerState.WEB_CONTENT)
    ...
    def test_any_x_input_stops_running_screensaver(self) -> None:
        ...
        ) as stop, mock.patch("chaski_player.daemon.time.monotonic", return_value=2):
